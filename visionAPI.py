...
class ResultData:
    def __init__(self, tanggal, total, quantity, type = "", name = "", custname = "", custaddress = "", price = "", listTransaction = None, product = "", merchant = "", awal = "", akhir = "", periode = ""):
    ...
    def __str__(self):
        return "Type: " + self.transactiontype + "BillingType: " + self.billingtype + "; Name: " + self.customerId + "; Custname: " + self.custname + "; Custaddress: " + self.custaddress + "; Price: " + self.rate + "; Quantity: " + self.quantity + "; Total: " + self.bill + "; tanggal: " + self.transactiondate + "; merchant: " + self.merchant + "; awal: " + self.awal + "; akhir: " + self.akhir + "; periode: " + self.periode 
    def to_json(self):
...
def get_noPelanggan(sLine):
    ...
    insensitive_No_Pelanggan = re.compile(re.escape('NOPEL/SAMB'), re.IGNORECASE)
    tempLine = insensitive_No_Pelanggan.sub('', tempLine)
...
def get_name_pdam(sLine):
    ...
    if x == -1:    
        ...
        insentive_name_pdam = re.compile(re.escape('PERIODE...'), re.IGNORECASE)
        tempLine = insentive_name_pdam.sub('', tempLine)
        
        insentive_name_pdam = re.compile(re.escape('ERIODE...'), re.IGNORECASE)
        tempLine = insentive_name_pdam.sub('', tempLine)
...
def get_merchant_indomart(sLine, textResult):
    tempLine = sLine

    escapeELA = re.compile(re.escape('SLIP'), re.IGNORECASE)
    tempLine = escapeELA.sub('', tempLine)

    escapeELA = re.compile(re.escape('PEMBAYARAN'), re.IGNORECASE)
    tempLine = escapeELA.sub('', tempLine)
...
def get_quantity(sLine):
    tempLine = sLine
    insensitive_liter = re.compile(re.escape('METER LALU'), re.IGNORECASE)
    tempLine = insensitive_liter.sub('', tempLine)
    
    insensitive_liter = re.compile(re.escape('METERLALU'), re.IGNORECASE)
    tempLine = insensitive_liter.sub('', tempLine)
...
def getMeterPdam(sLine):
    ...
    insensitive_liter = re.compile(re.escape('METER KINI'), re.IGNORECASE)
    tempLine = insensitive_liter.sub('', tempLine)
    
    insensitive_liter = re.compile(re.escape('METERKINI'), re.IGNORECASE)
    tempLine = insensitive_liter.sub('', tempLine)
    
    insensitive_liter = re.compile(re.escape('METER AWAL'), re.IGNORECASE)
    tempLine = insensitive_liter.sub('', tempLine)
    
    insensitive_liter = re.compile(re.escape('METERAWAL'), re.IGNORECASE)
    tempLine = insensitive_liter.sub('', tempLine)
    
    insensitive_liter = re.compile(re.escape('METER AKHIR'), re.IGNORECASE)
    tempLine = insensitive_liter.sub('', tempLine)
    
    insensitive_liter = re.compile(re.escape('METERAKHIR'), re.IGNORECASE)
    tempLine = insensitive_liter.sub('', tempLine)
    
    insensitive_liter = re.compile(re.escape('PETER AKHIR'), re.IGNORECASE)
    tempLine = insensitive_liter.sub('', tempLine)
    
    insensitive_liter = re.compile(re.escape('PETERAKHIR'), re.IGNORECASE)
    tempLine = insensitive_liter.sub('', tempLine)
...
def get_tanggal_BBM(sLine):
    ...
    insensitive_produk = re.compile(re.escape('TANGGAL'), re.IGNORECASE)
    tempLine = insensitive_produk.sub('', tempLine)
    insensitive_produk = re.compile(re.escape('TGL TRANS'), re.IGNORECASE)
    tempLine = insensitive_produk.sub('', tempLine)
...
def getTanggalShell(sLine):
    ...
    # tanggal = tanggal[:10] + " " + tanggal[-8:]
    print("getTanggalShell : " + tanggal)
    return tanggal

def getTanggalPdam(sLine):
    tempLine = sLine
    insensitive_sun = re.compile(re.escape('TANGGAL'), re.IGNORECASE)
    tempLine = insensitive_sun.sub('', tempLine)
    insensitive_mon = re.compile(re.escape('TANGGAL #'), re.IGNORECASE)
    tempLine = insensitive_mon.sub('', tempLine)
    tanggal = tempLine.strip()
    x = tanggal.find(":")
    # x = tanggal.find("#")
    ...
    if xtanggal_len == 10:
        match_str = re.search(r'\d{2}/\d{2}/\d{4}', x_split[1])
        res = datetime.strptime(match_str.group(), '%d-%m-%Y').date()
...
def validate_shell_date(date):
    try:
        # tempLine = date
        # tanggal_len = len(tempLine[0])
        # print(tempLine)
        
        # if tanggal_len == 10:
        #     match_str = re.search(r'\d{2}/\d{2}/\d{4}', date[0])
        #     res = datetime.strptime(match_str.group(), '%Y/%m/%d').date()
...
def validate_pdam_date(date):
    try:
        format = "%d-%m-%Y %H:%M:%S"
        # res = datetime.strptime(date, format)
        convBool = str(date)
        res = datetime.strptime(convBool, format).date()
        tanggal = res.strftime('%d-%m-%Y %H:%M:%S')
        # print("Does date match format? : " + str(res))
...
def validate_pdam_date_indomart(date):
    try:
        format = "%d/%m/%Y %H:%M:%S"
        # res = datetime.strptime(date, format)
        convBool = str(date)
...
def get_data(sLines, lines, textResult):
    ...
    for sLine in sLines:
        if "SPBU" in sLine.upper() or "PERTAMINA" in sLine.upper() or "VIVO" in sLine.upper() or "SHELL" in sLine.upper():
            type = "Bensin"
            readMethod = ReadMethod.InLine
            break
    ...
    if type == "Bensin": #Using textResult
        for i in range(len(textResult)):
            sLine = textResult[i]
            # sLine = sLine.replace(" ", "")
            if "PRODUK" in sLine.upper() or "JENIS BBM" in sLine.upper() or "NAMA PRODUK" in sLine.upper() or "PRODUCT" in sLine.upper():
                if readMethod == ReadMethod.InLine:
                    ...
                    field = "product"
            if "PRODUCT" in sLine.upper() or "GRADE" in sLine.upper():
                if readMethod == ReadMethod.InLine:
        ...
        for i in range(len(textResult)):
            ...
            # get merchant indomart
            if "METERKINI" in sLine.upper() or "METERLALU" in sLine.upper() :
                if readMethod == ReadMethod.InLine:
                    merchant = "INDOMARET"
                elif readMethod == ReadMethod.LineBelow:
                    field = "merchant"
                    
            # get merchant alfamart
            if "NOPEL/SAMB" in sLine.upper() or "REKENING" in sLine.upper():
                if readMethod == ReadMethod.InLine:
                    merchant = "ALFAMART"
                elif readMethod == ReadMethod.LineBelow:
                    field = "merchant"
                    
            # get merchant Pos Indonesia
            if "ADMIN POS" in sLine.upper() or "POS" in sLine.upper():
                if readMethod == ReadMethod.InLine:
                    merchant = "POS"
                elif readMethod == ReadMethod.LineBelow:
                    field = "merchant"
                    
            if "NAMA" in sLine.upper() or "NOPEL/SAMB" in sLine.upper():
                if readMethod == ReadMethod.InLine:
                    if product == "":
            ...
            if "TANGGAL" in sLine.upper() or "TANGGAL #" in sLine.upper(): 
                if readMethod == ReadMethod.InLine:
                    tanggal = getTanggalPdam(sLine)
                    len_tanggal = len(tanggal)
                    if len_tanggal == 10:
                        tanggal = get_waktu(sLines, tanggal)
                elif readMethod == ReadMethod.LineBelow:
                    field = "tanggal"

            if validate_pdam_date(sLine.upper()):
                if readMethod == ReadMethod.InLine:
                    tanggal =  get_waktu(sLine, tanggal)
                elif readMethod == ReadMethod.LineBelow:
                    field = "tanggal"
                    
            if validate_pdam_date_indomart(sLine.upper()):
                if readMethod == ReadMethod.InLine:
                    tanggal =  get_waktu(sLine, tanggal)
                elif readMethod == ReadMethod.LineBelow:
                    field = "tanggal"
                    
            if "NO PEL/SAMB" in sLine.upper() or "NOPEL/SAMB" in sLine.upper() or "No. Pelanggan" in sLine.upper() or "NO PELANGGAN" in sLine.upper():
                if readMethod == ReadMethod.InLine:
                    name = get_noPelanggan(sLine)
                elif readMethod == ReadMethod.LineBelow:
                    field = "name"
                    
            if "METERLALU" in sLine.upper() or "METER LALU" in sLine.upper() or "METER AWAL" in sLine.upper() or "METERAWAL" in sLine.upper():
                if readMethod == ReadMethod.InLine:
                    awal = getMeterPdam(sLine)
                elif readMethod == ReadMethod.LineBelow:
                    field = "awal"
            
            if "METERKINI" in sLine.upper() or "METER KINI" in sLine.upper() or "METER AKHIR" in sLine.upper() or "METERAKHIR" in sLine.upper() or "PETERAKHIR" in sLine.upper() or "PETER AKHIR" in sLine.upper():
                if readMethod == ReadMethod.InLine:
                    akhir = getMeterPdam(sLine)
                elif readMethod == ReadMethod.LineBelow:
                    field = "akhir"
                                              
            # if GetDate(sLine.upper()) == "PERIODE..." in sLine.upper() or "ERIODE..." in sLine.upper() or "TAGIHAN" in sLine.upper():
            #     if readMethod == ReadMethod.InLine:
            #         periode = GetDate_Nokey(sLine, tanggal)
            #     elif readMethod == ReadMethod.LineBelow:
            #         field = "periode"
                    
            if GetDate(sLine.upper()) == True:
                if readMethod == ReadMethod.InLine:
                    periode = GetDate_Nokey(sLine, tanggal)
                elif readMethod == ReadMethod.LineBelow:
                    field = "periode"
                    
            # if "PERIODE..." in sLine.upper() or "ERIODE..." in sLine.upper() or "TAGIHAN" in sLine.upper():
            #     if readMethod == ReadMethod.InLine:
            #         periode = get_name_pdam(sLine)
            #     elif readMethod == ReadMethod.LineBelow:
            #         field = "periode"
                    
            # if "PERIODE..." in sLine.upper() or "ERIODE..." in sLine.upper():
            #     if readMethod == ReadMethod.InLine:
            #         periode = get_name(sLines)
            #     elif readMethod == ReadMethod.LineBelow:
            #         field = "periode"
    ...
    if type == "Bensin": #Using sLines
        for i in range(len(sLines)):
            sLine = sLines[i]
            # sLine = sLine.replace(" ", "")
            if "PRODUK" in sLine.upper() or "JENIS BBM" in sLine.upper() or "NAMA PRODUK" in sLine.upper() or "GRADE" or "PRODUCT" in sLine.upper():
                if readMethod == ReadMethod.InLine:
                    if product == "":
                        product = get_product_BBM(sLine)
                    # custname = get_custname(sLines[i-2], textResult)
            ...
            if "TANGGAL" in sLine.upper() or "WAKTU" in sLine.upper() or "AKTU" in sLine.upper() or "DATE" in sLine.upper(): 
                if readMethod == ReadMethod.InLine:
                    if tanggal == "":
                        tanggal = get_tanggal_BBM(sLine)
                        len_tanggal = len(tanggal)
    ...
    if type == "Pdam_Postpaid": #Using sLines
        for i in range(len(sLines)):
            ...
            if "TAGIHAN" in sLine.upper() or "TOTAL" in sLine.upper() or "TOTAL TAGIHAN" in sLine.upper():
                if readMethod == ReadMethod.InLine:
                    if price == "":
                        price = get_price(sLine)
                elif readMethod == ReadMethod.LineBelow:
            ...
            # if isVolume(sLine.replace(" ", "")) == True or "METER KINI" in sLine.upper() or "METERKINI" in sLine.upper() or "METER AKHIR" in sLine.upper() or "METERAKHIR" in sLine.upper() or "PETER AKHIR" in sLine.upper() or "PETERAKHIR" in sLine.upper():
            #     if('HARGA/LITER' not in sLine.upper()):
            #         if readMethod == ReadMethod.InLine:
            #             if akhir == "":
            #                 akhir = getMeterPdam(sLine)
            ...
            # if isVolume(sLine.replace(" ", "")) == True or "METER LALU" in sLine.upper() or "METERLALU" in sLine.upper() or "METER AWAL" in sLine.upper() or "METERAWAL" in sLine.upper():
            #     if('HARGA/LITER' not in sLine.upper()):
            #         if readMethod == ReadMethod.InLine:
            #             if awal == "":
            #                 awal = getMeterPdam(sLine)
            ...
            if "NO PEL/SAMB" in sLine.upper() or "NO. PELANGGAN" in sLine.upper() or "NO PELANGGAN" in sLine.upper()  or "NOPEL/SAMB" in sLine.upper():
                if readMethod == ReadMethod.InLine:
                    if name == "":
                        name = get_noPelanggan(sLine)
                elif readMethod == ReadMethod.LineBelow:
            ...
            # if GetDate(sLine.upper()) == True or "PERIODE..." in sLine.upper() or "ERIODE" in sLine.upper()  or "TAGIHAN" in sLine.upper():
            #     if readMethod == ReadMethod.InLine:
            #         if periode == "":
            #             periode = GetDate_Nokey(sLine, periode)
            #     elif readMethod == ReadMethod.LineBelow:
            #         field = "periode"
                    
    result = ResultData(tanggal, total, quantity, type, name, custname, custaddress, price, listTransaction, product, merchant, awal, akhir, periode)
