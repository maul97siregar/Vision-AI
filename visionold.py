...
class ResultData:
    ...
    def __str__(self):
        return "Type: " + self.type + "; Name: " + self.name + "; Custname: " + self.custname + "; Custaddress: " + self.custaddress + "; Price: " + self.price + "; Quantity: " + self.quantity + "; Total: " + self.total + "; tanggal: " + self.tanggal
    def to_json(self):
    ...
    if readMethod == ReadMethod.Grid:
        for index,sWord in enumerate(sWords):
            ...
            #     name = sWord
                ...
                tempPrice = sWord.upper().replace("PRABAYAR", "").replace("PASCABAYAR", "").replace("/", "").replace(" ", "")
                price = ""
                digitCounter = 0
                for i in range(len(tempPrice)):
                    if tempPrice[i].isdigit():
                        digitCounter = digitCounter + 1
                        if digitCounter > 1:
                            price = price + "/" + tempPrice[i:]
                            break
                        else:
                            price = price + tempPrice[i]
                    else:
